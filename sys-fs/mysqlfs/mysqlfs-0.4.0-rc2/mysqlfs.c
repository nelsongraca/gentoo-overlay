...
static int mysqlfs_opt_proc(void *data, const char *arg, int key,
    ...
    if(!strncmp(arg, "host=", strlen("host="))){
        str = strchr(arg, '=') + 1;
        opt->host=malloc(sizeof(char)*(strlen(str))+1);
        strcpy(opt->host,str);
        //opt->host = str;
        return 0;
    }
    ...
    if(!strncmp(arg, "user=", strlen("user="))){
        str = strchr(arg, '=') + 1;
        opt->user=malloc(sizeof(char)*(strlen(str))+1);
        strcpy(opt->user,str);
//        opt->user = str;
        return 0;
    }
    ...
    if(!strncmp(arg, "password=", strlen("password="))){
        str = strchr(arg, '=') + 1;
        opt->passwd=malloc(sizeof(char)*(strlen(str))+1);
        strcpy(opt->passwd,str);
//        opt->passwd = str;
        return 0;
    }
    ...
    if(!strncmp(arg, "database=", strlen("database="))){
        str = strchr(arg, '=') + 1;
        opt->db=malloc(sizeof(char)*(strlen(str))+1);
        strcpy(opt->db,str);
//        opt->db = str;
        return 0;
    }
    ...
    if(!strncmp(arg, "socket=", strlen("socket="))){
        str = strchr(arg, '=') + 1;
        opt->socket=malloc(sizeof(char)*(strlen(str))+1);
        strcpy(opt->socket,str);
//        opt->socket = str;
        return 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    //we have to add this option, we need it
